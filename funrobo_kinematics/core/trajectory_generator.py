...
class MultiAxisTrajectoryGenerator:
    ...
    def plot(self):
        ...
        self.fig.suptitle(
            self.mode + " Trajectory Generator (Point-to-Point)", fontsize=16
        )
...
class MultiSegmentTrajectoryGenerator:
    ...
    def __init__(self, method=None, mode="joint", ndof=1):
        """
        Initialize the trajectory generator with the given configuration.
        """
        if method is None:
    ...
    def solve(self, waypoints, T):
        ...
        for i in range(self.n_segments):
            ...
            model = type(self.method)(
                ndof=self.ndof
            )  # creates a new instance of the trajectory gen class
    ...
    def generate(self, nsteps_per_segment=100):
        ...
        total_nsteps = (
            nsteps_per_segment - 1
        ) * self.n_segments + 1  # to avoid duplicate points at segment boundaries
